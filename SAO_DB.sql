...
-- -----------------------------------------------------
-- procedure SetEnrollmentArchived
-- -----------------------------------------------------

DELIMITER $$
USE `sao_db`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SetEnrollmentArchived`(
    IN p_Enrollment_ID INT,
    IN p_Archived TINYINT(1)
)
BEGIN
    IF NOT EXISTS (SELECT 1 FROM ENROLLMENT WHERE ID = p_Enrollment_ID) THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Enrollment record not found for archiving.';
    END IF;
...
-- 1. Modify StudentEnroll to grab the real ID and return it
DROP PROCEDURE IF EXISTS `StudentEnroll`;
DELIMITER $$
...
BEGIN
    ...
    SELECT curr.ID INTO v_curr_id FROM curriculum curr 
    JOIN program p ON curr.PROGRAM_ID = p.ID 
    JOIN course c ON curr.COURSE_ID = c.ID
    WHERE p.programName = PROGRAM_NAME AND c.Code = COURSE_CODE
      AND curr.YEAR_ID = p_YEAR_ID AND curr.SEMESTER_ID = p_SEMESTER_ID;

    IF v_student_id IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Enrollment Failed: Student name not found.';
    ELSEIF v_curr_id IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Enrollment Failed: No curriculum entry matches this Year/Semester/Course.';
    ELSE
        INSERT INTO enrollment (Grade, Status, Created_By, Updated_By, STUDENT_ID, CURRICULUM_ID)
        VALUES ('(Ongoing)', 'Active', 'registrar', 'registrar', v_student_id, v_curr_id);
        
        -- Retrieve the auto-incremented ID generated by the INSERT statement above
        SELECT LAST_INSERT_ID() AS newInsertId;
    END IF;
END$$
DELIMITER ;

-- 2. Revert SetEnrollmentArchived back to strictly using the exact ID
DROP PROCEDURE IF EXISTS `SetEnrollmentArchived`;
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SetEnrollmentArchived`(
    IN p_Enrollment_ID INT,
    IN p_Archived TINYINT(1)
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM ENROLLMENT WHERE ID = p_Enrollment_ID) THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Enrollment record not found for archiving.';
    END IF;

    UPDATE ENROLLMENT
    SET Is_Archived = p_Archived, Updated_At = NOW(), Updated_By = 'registrar'
    WHERE ID = p_Enrollment_ID;
END$$
DELIMITER ;
